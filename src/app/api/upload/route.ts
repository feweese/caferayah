...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user?.role !== "ADMIN" && user?.role !== "SUPER_ADMIN") {
      console.log(`Upload rejected: User role (${user?.role}) is insufficient`);
      return NextResponse.json(
    ...
    console.log("User has sufficient permissions for upload");
    ...
    return NextResponse.json(
      { 
        error: "Server error", 
        details: error instanceof Error ? error.message : "Failed to process image upload" 
      },
      { status: 500 }
    );
  }
